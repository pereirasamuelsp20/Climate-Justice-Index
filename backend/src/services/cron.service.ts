...
const aggregateWeatherForRegion = async (region: string, coords: { lat: number, lon: number }) => {
  ...
  try {
     ...
     if (owmJson.main?.temp > 40) owmData = 'Extreme Heat';
     else if (owmJson.wind?.speed > 25) owmData = 'Storm';
     else if (owmJson.weather?.[0]?.main === 'Rain') owmData = 'Rain';
  } catch (e) { logger.warn('OWM fetch failed'); }
  ...
  try {
     ...
     if (tmrJson.data?.values?.temperature > 40) tmrData = 'Extreme Heat';
     else if (tmrJson.data?.values?.windSpeed > 25) tmrData = 'Storm';
     else if (tmrJson.data?.values?.precipitationIntensity > 5) tmrData = 'Rain';
  } catch (e) { logger.warn('Tomorrow.io fetch failed'); }
  ...
  try {
     ...
     if (meteoJson.current_weather?.temperature > 40) meteoData = 'Extreme Heat';
     else if (meteoJson.current_weather?.windspeed > 90) meteoData = 'Storm'; // Open meteo windspeed is km/h
  } catch (e) { logger.warn('OpenMeteo fetch failed'); }
  ...
  for (const alertType of activeAlerts) {
    ...
    if (hits > 0) {
      ...
      if (confidence === 1) severity = 'Critical';
