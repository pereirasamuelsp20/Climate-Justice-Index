...
climateRouter.get('/countries/:iso2/regions', async (req, res, next) => {
    try {
        ...
        let countryRegions = allData.features.filter((f) => f.properties.iso_a2?.toUpperCase() === iso2.toUpperCase());
        // Determine base AQI
        ...
        const enrichedFeatures = countryRegions.map((f) => {
...
climateRouter.get('/aqi', async (req, res, next) => {
    try {
        ...
        const results = await Promise.allSettled(TRACKED_CITIES.map(async ({ iso2, city, lat, lon }) => {
        ...
        try {
            ...
            if (allRegions && allRegions.features) {
                const uniqueIso2s = new Set();
                allRegions.features.forEach((f) => {
                    if (f.properties?.iso_a2)
                        uniqueIso2s.add(f.properties.iso_a2.toUpperCase());
                });
...
// GET /climate/weather
climateRouter.get('/weather', async (req, res, next) => {
    try {
...
climateRouter.get('/initiatives', async (req, res, next) => {
    try {
