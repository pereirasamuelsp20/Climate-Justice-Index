...
export default function BarChart() {
  ...
  // Sort all data by selected metric descending
  ...
  // Build segment options (1–10, 11–20, 21–30, ...)
  const segmentOptions = useMemo(() => {
    const totalCountries = chartData.length;
    const options: { value: string; label: string }[] = [];
    for (let i = 0; i < totalCountries; i += PAGE_SIZE) {
      const start = i + 1;
      const end = Math.min(i + PAGE_SIZE, totalCountries);
      options.push({ value: String(i), label: `${start}–${end}` });
    }
    return options;
  }, [chartData.length]);

  // Get the current page of data
  const segmentStart = parseInt(selectedSegment) || 0;
  const pagedData = chartData.slice(segmentStart, segmentStart + PAGE_SIZE);
    ...
    <div className="flex flex-col h-full w-full relative" style={{ minHeight: 300 }}>
      ...
      <div className="flex flex-wrap justify-between items-center gap-3 text-xs mb-4 z-10 w-full pr-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-slate-600">of {chartData.length} countries</span>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <select 
            ...
            onChange={(e) => { setSelectedMetricId(e.target.value as MetricId); setSelectedSegment('0'); }}
            className="bg-[#1a1d2e] border border-white/10 text-slate-300 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block px-3 py-2 outline-none cursor-pointer"
          >
            {METRICS.map(m => (
        ...
        >
          ...
          <YAxis 
            ...
            tickFormatter={(val) => {
              ...
              return val >= 1000000 ? `${(val / 1000000).toFixed(1)}M` : val >= 1000 ? `${(val / 1000).toFixed(1)}k` : String(val);
            }}
          />
          <Tooltip 
